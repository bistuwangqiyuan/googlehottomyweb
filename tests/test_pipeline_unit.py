...
def test_review_config_atomic_no_mix():
    """部分 REVIEW_* 残留（有 MODEL 无 KEY）时必须整套借用 LLM_*，
    不得把 REVIEW_MODEL 混进 LLM 端点（跨厂商模型名必然 400）。"""
    from pipeline.review import review_config
    saved = {k: os.environ.get(k) for k in
             ("LLM_API_KEY", "LLM_BASE_URL", "LLM_MODEL",
              "REVIEW_API_KEY", "REVIEW_BASE_URL", "REVIEW_MODEL")}
    try:
        os.environ["LLM_API_KEY"] = "k1"
        os.environ["LLM_BASE_URL"] = "https://a.example/v1"
        os.environ["LLM_MODEL"] = "model-a"
        os.environ.pop("REVIEW_API_KEY", None)
        os.environ.pop("REVIEW_BASE_URL", None)
        os.environ["REVIEW_MODEL"] = "model-b-from-other-vendor"
        cfg = review_config()
        assert cfg["model"] == "model-a" and cfg["base_url"] == "https://a.example/v1", cfg
        # 配了完整 REVIEW_* 时则用 REVIEW_* 三元组
        os.environ["REVIEW_API_KEY"] = "k2"
        os.environ["REVIEW_BASE_URL"] = "https://b.example/v1"
        cfg = review_config()
        assert cfg["model"] == "model-b-from-other-vendor" and cfg["api_key"] == "k2", cfg
    finally:
...
def test_offline_pipeline_end_to_end():
    with tempfile.TemporaryDirectory() as d:
        content = Path(d) / "briefings"
        audit = Path(d) / "audit"
        # 净化环境：剥离 LLM/REVIEW key，强制确定性 briefing 模式
        # （单元测试不依赖网络与外部 API，稳定可重复）
        clean_env = {k: v for k, v in os.environ.items()
                     if not (k.startswith("LLM_") or k.startswith("REVIEW_"))}
        proc = subprocess.run(
            [sys.executable, "-m", "pipeline.run_pipeline",
             "--fixture-dir", str(FIXTURES),
             "--content-dir", str(content), "--audit-dir", str(audit),
             "--max-publish", "5"],
            cwd=ROOT, capture_output=True, text=True, timeout=300, env=clean_env,
        )
