...
**方法**（`scripts/02_trend_lifecycle.py`，[D2]）：在 2025 年每月取一个采样日，抓取英文维基百科当日浏览量 Top50 文章（去重后 431 篇候选，其中 2 个采样日 API 未返回数据、133 篇因数据不足剔除，剩 298 篇有效候选——如实披露样本损耗）；对每篇拉取 ±60 天逐日浏览量，以"峰值/基线中位数 ≥5 倍"判定事件驱动型热点，共识别 **278 个热点**；对每个热点计算衰减指标。维基浏览量为公众注意力的开放代理指标（与搜索热度同源，局限性讨论见第一章 1.4）。
...
| 衰减至 10% 所需天数 | 4 天 | 3 天 | 15 天 |
| 峰值起前 3 天占 30 天窗口超额注意力的比例 | **64.8%** | 34.7% | 80.5% |
| Top50 中事件驱动型占比 | 93.3%（278/298） | — | — |
...
1. **窗口以天计，半数注意力在 48 小时内蒸发。** 中位热点的超额注意力半衰期 2 天、前 3 天拿走 65% 的 30 天总量。传统内容工作流（选题→撰写→编辑→发布，典型 2–5 天）到达时窗口已关闭大半——**这解释了为什么该机会至今没有被传统内容团队吃掉：不是没人看见，是人追不上。**
2. **自动化的价值可量化。** 从热词出现到内容发布，每提前一天，可捕获的窗口注意力份额显著提升（首日发布 vs 第 3 天发布，按中位衰减曲线差距约一倍）。分钟级流水线不是锦上添花，是该市场的入场券。
3. **长尾同样存在。** P75 衰减至 10% 需 15 天，且部分热点（曲线图中高位长尾）演变为持续话题——引擎应配"常青化改写"模块，将验证过流量的热点内容改写为常青内容，延长资产寿命，摊薄单篇成本。
